...
float f(float x)
{
	// we are taking equation as x^3+x-11
...
{
	float n = 0, xm, x0, c;// root er kotota kachakachi gechi seta mapar (new guess) ,current approximation of the root, test if oot is found
	if (f(x1) * f(x2) < 0) {
		do {
			...
			c = f(x1) * f(x0);// x0 actually root kina seta janar jonno
